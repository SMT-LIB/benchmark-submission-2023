...
(assert (not (= ~head~0.base (select (select |#memory_$Pointer$.base| ~head~0.base) 4))))
...
(assert (= (select (select |#memory_$Pointer$.offset| ~head~0.base) 4) 0))
(assert (= (select |#valid| ~head~0.base) 1))
(assert (<= 5 (select (select |#memory_int| (select (select |#memory_$Pointer$.base| ~head~0.base) 4)) 0)))
(assert (= insert_list_~l.offset 0))
(assert (not (= 2 (select (select |#memory_int| ~head~0.base) 0))))
(assert (= |insert_list_#in~k| insert_list_~k))
(assert (= (store |old(#valid)| insert_list_~l.base 1) |#valid|))
(assert (= (select |#valid| (select (select |#memory_$Pointer$.base| ~head~0.base) 4)) 1))
...
(assert (not (exists ((v_ArrVal_1070 (Array Int Int))) (= |#memory_int| (store |old(#memory_int)| insert_list_~l.base v_ArrVal_1070)))))
(check-sat)
(exit)
