...
(assert (exists ((v_arrayElimArr_11 (Array Int Int))) (and (= |old(~head~0.offset)| (select v_arrayElimArr_11 4)) (= |#memory_$Pointer$.offset| (store |old(#memory_$Pointer$.offset)| ~head~0.base v_arrayElimArr_11)))))
(assert (= (store |old(#valid)| ~head~0.base 1) |#valid|))
(assert (= ~head~0.offset 0))
(assert (exists ((v_arrayElimArr_10 (Array Int Int))) (and (= (store |old(#memory_$Pointer$.base)| ~head~0.base v_arrayElimArr_10) |#memory_$Pointer$.base|) (= |old(~head~0.base)| (select v_arrayElimArr_10 4)))))
(assert (= (select |old(#valid)| ~head~0.base) 0))
(assert (not (exists ((v_ArrVal_1686 (Array Int Int))) (= |#memory_int| (store |old(#memory_int)| ~head~0.base v_ArrVal_1686)))))
(check-sat)
(exit)
