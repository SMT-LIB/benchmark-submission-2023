...
(assert (= |old(~head~0.base)| (select (select |#memory_$Pointer$.base| ~head~0.base) 4)))
(assert (= (store |old(#valid)| ~head~0.base 1) |#valid|))
(assert (= ~head~0.offset 0))
(assert (not (exists ((v_ArrVal_1075 (Array Int Int))) (= (store |old(#memory_int)| ~head~0.base v_ArrVal_1075) |#memory_int|))))
(check-sat)
(exit)
