...
(assert (not (= |c_ULTIMATE.start_main_~expected~0#1| (let ((.cse0 (bvadd |c_ULTIMATE.start_sll_get_data_at_~head#1.offset| (_ bv4 64)))) (select (select |c_#memory_int| (select (select |c_#memory_$Pointer$.base| |c_ULTIMATE.start_sll_get_data_at_~head#1.base|) .cse0)) (select (select |c_#memory_$Pointer$.offset| |c_ULTIMATE.start_sll_get_data_at_~head#1.base|) .cse0))))))
(check-sat)
(exit)
