...
(assert (or (<= (* oldres oldres) (+ x oldres)) (<= (* oldres oldres) (+ x (- (/ x 10751840) (uninterp_frac x 10751840))))))
(assert (= newres (- (/ (+ oldres (- (/ x oldres) (uninterp_frac x oldres))) 2) (uninterp_frac (+ oldres (- (/ x oldres) (uninterp_frac x oldres))) 2))))
(assert (not (and (< x (* (+ newres 1) (+ newres 1))) (or (< (- (* newres newres) 1) (+ x newres)) (< (- (* newres newres) 1) (+ x (- (/ x 462408296549760) (uninterp_frac x 462408296549760))))))))
(check-sat)
(exit)
