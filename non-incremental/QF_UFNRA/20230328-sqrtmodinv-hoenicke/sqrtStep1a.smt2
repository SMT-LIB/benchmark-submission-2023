...
(assert (axiom_frac_zero (* 25 x) 16 4 (* oldres oldres) (uninterp_frac (* 25 x) 16)))
...
(assert (not (and (< x (* (+ newres 1) (+ newres 1))) (or (< (- (* newres newres) 1) (+ x newres)) (< (- (* newres newres) 1) (- (/ (* 25 x) 16) (uninterp_frac (* 25 x) 16)))))))
(check-sat)
(exit)
