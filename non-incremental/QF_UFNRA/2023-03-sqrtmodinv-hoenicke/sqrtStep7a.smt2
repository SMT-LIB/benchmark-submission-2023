...
(assert (or (<= (* oldres oldres) (+ x oldres)) (<= (* oldres oldres) (+ x (- (/ x 855285730872204993313810429440) (uninterp_frac x 855285730872204993313810429440))))))
(assert (= newres (- (/ (+ oldres (- (/ x oldres) (uninterp_frac x oldres))) 2) (uninterp_frac (+ oldres (- (/ x oldres) (uninterp_frac x oldres))) 2))))
(assert (not (and (< x (* (+ newres 1) (+ newres 1))) (or (< (- (* newres newres) 1) (+ x newres)) (< (- (* newres newres) 1) (+ x (- (/ x 2926054725734407478371345979251228656221734313834524116572160) (uninterp_frac x 2926054725734407478371345979251228656221734313834524116572160))))))))
(check-sat)
(exit)
